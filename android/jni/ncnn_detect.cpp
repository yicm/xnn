                     ...
                     AAssetManager *mgr)
{
    ...
    if (param_path.size() != 0 && bin_path.size() != 0)
    {
        ...
        {
            if (mgr != NULL) {
                net_.load_param_bin(mgr, param_path.c_str());
            } else {
        ...
        {
            if (mgr != NULL) {
                net_.load_param(mgr, param_path.c_str());
            } else {
                 net_.load_param(param_path.c_str());
            }
        }
        if (mgr != NULL) {
            net_.load_model(mgr, bin_path.c_str());
        } else {
...
XNNStatus NCNNDetect::run(XNNImage *image, std::vector<DetectObject>& objects, int topk)
{
    ...
    if (image->width == input_size_ && image->height == input_size_) {
        in = ncnn::Mat::from_pixels(image->data, XNN_PIX_RGB, input_size_, input_size_);
    } else {
        in = ncnn::Mat::from_pixels_resize(image->data,XNN_PIX_RGB, image->width, image->height, w, h);
    }
...
//        LOGD("idx:%d ==>宽 %.0f --> %.0f  高: %.0f --> %.0f ", i, x0, x1, y0, y1);
