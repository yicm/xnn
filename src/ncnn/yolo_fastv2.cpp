...
//ncnn 模型加载
int yoloFastestv2::loadModel(const char* paramPath, const char* binPath, int targetSize)
{
...
int yoloFastestv2::detection(XNNImage *image, std::vector<TargetBox> &dstBoxes, const float thresh)
{
    dstBoxes.clear();

    float scaleW = (float)image->width / (float)inputWidth;
    float scaleH = (float)image->height / (float)inputHeight;

    fprintf(stdout, "imageW=%d, imageH=%d, targetSize=%d\n", image->width, image->height, inputWidth);

    //resize of input image data
    ncnn::Mat inputImg = ncnn::Mat::from_pixels_resize(image->data, ncnn::Mat::PIXEL_BGR,\
                                                       image->width, image->height, inputWidth, inputHeight);
